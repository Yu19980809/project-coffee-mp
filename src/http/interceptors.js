      ...
      switch (res.statusCode) {
        ...
        case config.AUTHENTICATE:
          ...
          if (isTokenRefreshing) {
            const {url: u, method, params, needToken, header} = requestParams
            return failedRequests.push(() => request(u, method, params, needToken, true, header))
          }
          isTokenRefreshing = true
          const url = '/auth/refresh-token/weapp'
          ...
          return request(url, 'POST', {refreshToken}, false)
            .then(response => {
              ...
              // 再将之前报 401 错误的请求重新发送
              const {url: u, method, params, needToken, header} = requestParams
              return request(u, method, params, needToken, true, header)
            })
